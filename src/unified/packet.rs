...
/// 从旧的Packet结构转换为新的UnifiedPacket
impl From<crate::packet::Packet> for UnifiedPacket {
    fn from(old_packet: crate::packet::Packet) -> Self {
        // 从旧的header中提取信息
        let packet_type = PacketType::from((old_packet.header.message_id % 256) as u8);
        let message_id = old_packet.header.message_id;
        
        // 合并extend_header和payload
        let mut payload = BytesMut::new();
        payload.extend_from_slice(&old_packet.extend_header);
        payload.extend_from_slice(&old_packet.payload);
...
/// 从新的UnifiedPacket转换为旧的Packet结构（用于兼容）
impl From<UnifiedPacket> for crate::packet::Packet {
    fn from(unified_packet: UnifiedPacket) -> Self {
        use crate::packet::{PacketHeader};
        use crate::compression::CompressionMethod;
        
        let extend_header = BytesMut::from(&[unified_packet.packet_type.into()][..]);
        let header = PacketHeader {
            message_id: unified_packet.message_id,
            message_length: unified_packet.payload.len() as u32,
        ...
        Self {
            header,
            extend_header,
            payload: unified_packet.payload,
        }
    }
}
