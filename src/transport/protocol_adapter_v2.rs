...
mod tests {
    ...
    #[tokio::test]
    async fn test_flume_protocol_adapter_basic() {
        ...
        let (adapter, _event_rx) = FlumePoweredProtocolAdapter::new(1, connection_info);
        
        // 测试非阻塞发送
        let packet = create_test_packet(1, 1024);
        assert!(adapter.send_nowait(packet).is_ok());
        
        // 测试批量处理
        let processed = adapter.process_send_batch(10).await.unwrap();
    ...
    async fn test_batch_processing_performance() {
        ...
        while total_processed < 100 {
            let processed = adapter.process_send_batch(32).await.unwrap();
            if processed == 0 {
                break;
            }
