...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🚀 新架构 QUIC 客户端测试（直接broadcast版本）");
    ...
    // 直接访问broadcast通道（类似WebSocket direct的做法）
    let mut event_rx = receiver.event_rx;
    
    // 启动接收任务（直接使用broadcast通道）
    tokio::spawn(async move {
        println!("🚀 QUIC直接broadcast接收任务已启动...");
        while let Ok(event) = event_rx.recv().await {
            use msgtrans::transport::quic::QuicEvent;
            match event {
                QuicEvent::PacketReceived(packet) => {
                    println!(
                        "🎉 收到QUIC服务器回复! ID: {}, 载荷: {:?}",
